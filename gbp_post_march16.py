...
async def _click_skip_in_copy_dialog(page, key="gbp"):
    ...
    log(key, "STEP: Dismissing 'Copy post' dialog (clicking Skip in iframe)...")
    ...
    for attempt in range(3):
        for frame in page.frames:
            try:
                ...
                if result:
                    log(key, "SUCCESS: Clicked Skip in iframe — post is now published")
        ...
        try:
            ...
            if result:
                log(key, "SUCCESS: Clicked Skip on main page — post is now published")
                await page.wait_for_timeout(3000)
                return True
        except Exception:
            pass

        log(key, f"Skip not found yet (attempt {attempt+1}/3), waiting...")
        await page.wait_for_timeout(2000)

    log(key, "FAILED: Could not find Skip button after 3 attempts — POST MAY NOT BE PUBLISHED")
    return False
...
async def post_one(post: dict) -> bool:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            log(key, "Clicking Post button (opens Copy dialog)...")
            try:
                post_btn = modal_frame.locator('button:has-text("Post")').last
                await post_btn.wait_for(timeout=5000)
                await post_btn.click()
                log(key, "Clicked Post — now need to handle Copy dialog")
            except Exception as e:
            ...
            await page.screenshot(path=str(SCRIPT_DIR / f"gbp_{key}_step5_copy_dialog.png"))

            # CRITICAL: Click Skip in the Copy post dialog to actually publish
            published = await _click_skip_in_copy_dialog(page, key)

            await page.screenshot(path=str(SCRIPT_DIR / f"gbp_{key}_step6_final.png"), full_page=True)
            if published:
                log(key, "POST CONFIRMED — live on GBP")
            else:
                log(key, "WARNING: Post may not have published — check GBP manually")
            return published
